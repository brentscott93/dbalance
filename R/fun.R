...
#' Note: You need to convert time to seconds. The dbalance::to_seconds() function can convert time in MM:SS to seconds.
#'
...
calc_critical_speed <- function(pr_data){
  ...
  check_names <- colnames(pr_data) %in% c("athlete", "distance_meters", "seconds")
  if(!all(check_names)){
    stop("pr_data must be a table with only 3 columns whose names are 'athlete', 'distance_meters', and 'seconds'")
  }
...
#' @param runner_col_names FALSE (default) or a character vector with the names to be replaced.
...
#' Plot Lollipop Race
#'
#' @param runner_data a df of runners name, cs, and d' prime. Need to have colnames as specified in calc_d_balance().
